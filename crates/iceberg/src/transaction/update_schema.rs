...
pub struct UpdateSchemaAction {
    ...
    deletes: Vec<String>,
    // ARGON fork (Gap 4, plans/UPSTREAM-ICEBERG.md): columns to widen
    // `required` -> `optional`. Spec-legal, metadata-only (same field-id,
    // same data files); the one schema evolution ArgonDB's D14 nullability
    // corollary needs and that upstream 0.10's `add_column`/`delete_column`
    // does not expose. UPSTREAM CANDIDATE: `make_column_optional` is
    // generically useful (Java `UpdateSchema.makeColumnOptional` equivalent).
...
impl UpdateSchemaAction {
    ...
    /// ARGON fork (Gap 4): widen a `required` column to `optional` — the Java
...
/// ARGON fork (Gap 4): fields whose IDs appear in `make_optional_ids` are
/// re-emitted with `required = false` (metadata-only widening).
fn rebuild_fields(
...
/// ARGON fork (Gap 4): a field whose ID is in `make_optional_ids` is re-emitted
/// with `required = false`; already-optional fields are unaffected (idempotent).
fn rebuild_field(
    ...
    // ARGON fork (Gap 4): required -> optional widening. `&& !contains` makes an
    // already-optional field a no-op (stays false), so the operation is idempotent.
...
impl TransactionAction for UpdateSchemaAction {
    async fn commit(self: Arc<Self>, table: &Table) -> Result<ActionCommit> {
        ...
        // --- 1b. ARGON fork (Gap 4): resolve required->optional widenings ---
        ...
        // required fields). This is the D14 corollary's residual class: a
        // specials-capable PK stays `required` (plans/UPSTREAM-ICEBERG.md Gap 4).
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // ARGON fork (Gap 4, plans/UPSTREAM-ICEBERG.md): required -> optional
    // widening. The D14 nullability-corollary migration for pre-L9 lake tables
    // — spec-legal, metadata-only, no data rewrite. Java
    // `UpdateSchema.makeColumnOptional` equivalent. UPSTREAM CANDIDATE.
    // -----------------------------------------------------------------------
    ...
    #[tokio::test]
    async fn argon_make_column_optional_widens_required_column() {
        let table = make_v2_table();
        let tx = Transaction::new(&table);
    ...
    /// At the ArgonDB call site the detector (`specials_relax_plan`) returns `[]`
    /// once satisfied, so this no-op path is belt-and-suspenders.
    #[tokio::test]
    async fn argon_make_column_optional_already_optional_is_noop() {
        let table = make_v2_table_with_nested();
        let tx = Transaction::new(&table);
    ...
    /// The residual class the retained `RequiredNull` L3 trigger covers: an
    ...
    #[tokio::test]
    async fn argon_make_column_optional_refuses_identifier_field() {
        let table = make_v2_table();
        let tx = Transaction::new(&table);
    ...
    #[tokio::test]
    async fn argon_make_column_optional_missing_column_fails() {
        let table = make_v2_table();
        let tx = Transaction::new(&table);
